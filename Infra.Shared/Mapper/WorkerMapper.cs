...
namespace Infra.Shared.Mapper
{
    public static class WorkerMapper
    {
        ...
        public static IEnumerable<WorkerModel> ConvertToUsers(this IList<Worker> users) =>
            new List<WorkerModel>(users.Select(s => new WorkerModel(s.Id, s.Name.ToString(), s.BirthDate, s.Cpf.ToString())));

        public static WorkerModel ConvertToUser(this Worker user) =>
            new WorkerModel(user.Id, user.Name.ToString(), user.BirthDate, user.Cpf.ToString());
    }
}
